...
There is the area where [Xmf\Database\TableLoad](../reference/database/tableload.md) can assist.
...
Take the file describe in the previous step, and load it to an array using [Xmf\Yaml](../reference/yaml.md).
...
Loading is also more complicated, as we need to mark any existing help pages as inactive. For this, in this case, we will use the `update()` method from our [Xmf\Database\Tables](migrating-a-modules-database.md) companion class.
...
If your module requires initializing tables with data, [Xmf\Database\TableLoad](../reference/database/tableload.md) is a useful tool to keep in mind.
